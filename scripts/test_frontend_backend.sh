...
test_action() {
    ...
    response=$(eval "$api_call" | curl -s -L -w "\n%{http_code}")
...
if [ "$CREATE_CODE" -eq 201 ]; then
    ...
    echo -e "  ${RED}✗ Failed to create test estate plan${NC}"
    ((FAILED++))
    exit 1
fi
...
if [ "$BENEFICIARY_CODE" -eq 201 ]; then
    ...
    echo -e "  ${RED}✗ Failed to create beneficiary${NC}"
    ((FAILED++))
fi
...
RELATIONS_BODY=$(curl -s "${BASE_URL}/estate-plans/${ESTATE_PLAN_ID}")
